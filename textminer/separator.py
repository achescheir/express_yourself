...
def date(test_string):
    ...
    month_length = {1: 31, 3: 31, 4: 30, 5: 31, 6: 30, 7: 31,
                    8: 31, 9: 30, 10: 31, 11: 30, 12: 31}
    months = {'jan': 1, 'january': 1, 'feb': 2, 'february': 2,
              'mar': 3, 'march': 3}  # etc
    if matches:
        ...
        matches = re.match(r"^(\d{4})\s([a-zA-Z]+)\s(\d{2})$", test_string)
        # year named-month day
        if matches:
            return_dict = {}
            return_dict['year'] = int(matches.group(1))
...
def address(test_string):
    matches = re.match(r"(?P<address>\d+[^,\n]+)[,\n]\s*(?P<city>[a-zA-Z ]+)" +
                       r", (?P<state>(?:MS)|(?:AK)|(?:MT)|(?:NH)) " +
                       r"(?P<zip>\d{5})(?:-(?P<plus4>\d{1,4}))?$", test_string)
    if matches:
...
def email(test_string):
    matches = re.match(r"(?P<local>[\w\.]+)@(?P<domain>[\w]+\.[\w]+)$",
                       test_string)
    if matches:
        return matches.groupdict()
    else:
